...
zneupd(struct ARPACK_arnoldi_update_vars_d *V, int rvec, int howmny, int* select,
       ...
       double* workev, ARPACK_CPLX_TYPE* resid, ARPACK_CPLX_TYPE* v, int ldv,
       int* ipntr, ARPACK_CPLX_TYPE* workd, ARPACK_CPLX_TYPE* workl, double* rwork)
{
     ...
     *-----------------------------------*/
    rnorm = workl[ih+1];
    workl[ih+1] = ARPACK_cplx(0.0, 0.0);
    ...
    if (rvec) {
        ...
        zlaset_("A", &V->ncv, &V->ncv, &dbl0, &dbl1, &workl[invsub], ldq);
       ...
       ARPACK_CPLX_TYPE* workd, ARPACK_CPLX_TYPE* rwork)
{
    ...
    int i, ierr, initv, int1 = 1, j, nev0, np0, tmp_int;
    const double eps23 = pow(ulp, 2.0 / 3.0);
...
LINE20:
    ...
    for (i = 0; i < V->nev; i++)
    {
        rtemp = fmax(eps23, hypot(creal(ritz[V->np + i]), cimag(ritz[V->np + i])));
        if (hypot(creal(bounds[V->np + i]), cimag(bounds[V->np + i])) <= V->tol*rtemp)
        {
            V->nconv += 1;
        }
    }
    ...
    if ((V->nconv >= V->aup2_nev0) || (V->aup2_iter > V->maxiter) || (V->np == 0))
    {
        ...
        {
            temp = fmax(eps23, hypot(creal(ritz[j]), cimag(ritz[j])));
...
LINE100:
    ...
    if (V->bmat)
    {
        V->aup2_rnorm = cabs(zdotc_(&V->n, resid, &int1, workd, &int1));
        V->aup2_rnorm = sqrt(V->aup2_rnorm);
    } else {
        V->aup2_rnorm = dznrm2_(&V->n, resid, &int1);
    }
...
LINE60:
     ...
     *-----------------------------------------*/
    zgemv_(TRANS, &n, &V->aitr_j, &dbl1, v, &ldv, &workd[ipj], &int1, &dbl0, &h[ldh*V->aitr_j], &int1);
    ...
    TRANS = "N";
    tmp_dbl = -1.0;
    zgemv_(TRANS, &n, &V->aitr_j, &tmp_dbl, v, &ldv, &h[ldv*V->aitr_j], &int1, &dbl1, resid, &int1);
...
LINE80:
    ...
    TRANS = "T";
    zgemv_(TRANS, &n, &V->aitr_j, &dbl1, v, &ldv, &workd[ipj], &int1, &dbl0, &workd[irj], &int1);
    ...
    TRANS = "N";
    tmp_dbl = -1.0;
    zgemv_(TRANS, &n, &V->aitr_j, &tmp_dbl, v, &ldv, &workd[irj], &int1, &dbl1, resid, &int1);
    zaxpy_(&V->aitr_j, &dbl1, &workd[irj], &int1, &h[ldh*V->aitr_j], &int1);
    V->aitr_orth2 = 1;
    if (V->bmat)
    {
        zcopy_(&n, resid, &int1, &workd[irj], &int1);
...
LINE100:
    ...
    if (V->aitr_j >= V->nev + V->np)
    {
        ...
        for (i = k - 1; i < k - 2 + V->np; i++)
        {
            ...
            {
                tmp_int = k + V->np;
                tst1 = zlanhs_(NORM, &tmp_int, h, &ldh, &workd[n]);
            }
       ...
       ARPACK_CPLX_TYPE* workd)
{
    ...
    for (i = 0; i < kplusp; i++)
    {
        for (j = 0; j < kplusp; j++)
        {
            q[j + ldq*i] = ARPACK_cplx(0.0, 0.0);
            if (i == j) { q[j + ldq*i] = ARPACK_cplx(1.0, 0.0); }
        }
    }
...
LINE20:
        ...
        for (i = istart; i < kplusp - 1; i++)
        {
            ...
            tst1 = fabs(creal(h[i + ldh*i])) + fabs(cimag(h[i + ldh*i])) +
                   fabs(creal(h[i+1 + ldh*(i+1)])) + fabs(cimag(h[i+1 + ldh*(i+1)]));
            if (tst1 == 0.0)
            {
            ...
            if (fabs(creal(h[i+1 + ldh*i])) <= fmax(ulp*tst1, smlnum))
            {
                iend = i;
                h[i+1 + ldh*i] = ARPACK_cplx(0.0, 0.0);
                break;
        ...
        for (i = istart; i <= iend-1; i++)
        {
            ...
            // More MSVC nonsense to get basic arithmetic ops
            // MSVC does not even implement +,-,*,/ ops for complex numbers.
            for (j = i; j < kplusp; j++)
            {
                #if defined(_MSC_VER)
                    t = _Cmulcc(s, h[i + 1 + ldh*j]);
                    tmp_cplx = _Cmulcr(h[i + ldh*j], c);
                    t = ARPACK_cplx(creal(t) + creal(tmp_cplx), cimag(t) + cimag(tmp_cplx));

                    tmp_cplx  = _Cmulcc(ARPACK_cplx(-creal(s),cimag(s)), h[i + 1 + ldh*j]);
                    tmp_cplx2 = _Cmulcr(h[i + 1 + ldh*j], c);
                    h[i + 1 + ldh*j] = ARPACK_cplx(creal(tmp_cplx) + creal(tmp_cplx2), cimag(tmp_cplx)+cimag(tmp_cplx2));

                    h[i + ldh*j]     = t;
                #else
                    t                =        c*h[i + ldh*j] + s*h[i + 1 + ldh*j];
                    h[i + 1 + ldh*j] = -conj(s)*h[i + ldh*j] + c*h[i + 1 + ldh*j];
                    h[i + ldh*j]     = t;
                #endif
            }
            // 50
            ...
            {
                #if defined(_MSC_VER)
                    t = _Cmulcc(ARPACK_cplx(creal(s), -cimag(s)), h[j + ldh*(i + 1)]);
                    tmp_cplx = _Cmulcr(h[j + ldh*i], c);
                    t = ARPACK_cplx(creal(t) + creal(tmp_cplx), cimag(t) + cimag(tmp_cplx));

                    tmp_cplx  = _Cmulcc(ARPACK_cplx(-creal(s), -cimag(s)), h[j + ldh*i]);
                    tmp_cplx2 = _Cmulcr(h[j + ldh*(i + 1)], c);
                    h[j + ldh*(i + 1)] = ARPACK_cplx(creal(tmp_cplx) + creal(tmp_cplx2), cimag(tmp_cplx) + cimag(tmp_cplx2));

                    h[j + ldh*i]       = t;
                #else
                    t                  =  c*h[j + ldh*i] + conj(s)*h[j + ldh*(i + 1)];
            ...
            {
                #if defined(_MSC_VER)
                    t = _Cmulcc(ARPACK_cplx(creal(s), -cimag(s)), q[j + ldq*(i + 1)]);
                    tmp_cplx = _Cmulcr(q[j + ldq*i], c);
                    t = ARPACK_cplx(creal(t) + creal(tmp_cplx), cimag(t) + cimag(tmp_cplx));

                    tmp_cplx  = _Cmulcc(ARPACK_cplx(-creal(s), -cimag(s)), q[j + ldq*i]);
                    tmp_cplx2 = _Cmulcr(q[j + ldq*(i + 1)], c);
                    q[j + ldq*(i + 1)] = ARPACK_cplx(creal(tmp_cplx) + creal(tmp_cplx2), cimag(tmp_cplx) + cimag(tmp_cplx2));

                    q[j + ldq*i]       = t;
                #else
                    t                  =  c*q[j + ldq*i] + conj(s)*q[j + ldq*(i + 1)];
    ...
    for (i = 0; i < *kev; i++)
    {
        ...
        if (tst1 == 0.0)
        {
            tst1 = zlanhs_(NORM, kev, h, &ldh, workl);
        }
    ...
    {
        zgemv_(TRANS, &n, &kplusp, &dbl1, v, &ldv, &q[(*kev)*ldq], &int1, &dbl0, &workd[n], &int1);
    }
    ...
    {
        tmp_int = kplusp - i;
        zgemv_(TRANS, &n, &tmp_int, &dbl1, v, &ldv, &q[(*kev-i)*ldq], &int1, &dbl0, workd, &int1);
       ...
       double* rwork, int* ierr)
{
    ...
    char *UPLO = "A", *SIDE = "R", *HOWMNY = "B", *TRANS = "T";
       ...
       ARPACK_CPLX_TYPE* ritz, ARPACK_CPLX_TYPE* bounds)
{
    ...
    if (V->ishift == 1)
    {
         ...
         *------------------------------------------------------*/
        zsortc(which_SR, 1, *np, bounds, ritz);
    }
...
void
zgetv0(struct ARPACK_arnoldi_update_vars_d *V, const int initv, const int n, const int j,
...
LINE20:
    ...
    if (V->bmat)
    {
        cnorm = zdotc_(&n, resid, &int1, workd, &int1);
        V->getv0_rnorm0 = sqrt(cabs(cnorm));
    } else {
...
LINE30:
    zgemv_(TRANS, &n, &j, &int1, v, &ldv, workd, &int1, &int0, &workd[n], &int1);
    TRANS = "N";
    zgemv_(TRANS, &n, &j, &intm1, v, &ldv, workd, &int1, &int1, &workd[n], &int1);
